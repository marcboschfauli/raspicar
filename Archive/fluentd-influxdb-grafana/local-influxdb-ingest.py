...
You must supply the path to said log file and as well as a timezone offset
in hours (set to 0 if the ingest source time stamps are in the same timezone
as the InfluxDB server).

Usage: 
     
  ` python local-influxdb-ingest.py test-piin.log.test 1`
...
DATABASE = 'test' # InfluxDB target database
...
#
#  {"message":"2014-09-12 13:29:43.513393;123;776;0.0;17.25;31.764;1000"}
#
# with the following message schema:
#
#  Timestamp;VehicleID;RPM;MPH;Throttle;Load;Fuel_Status
#
def parse_line(line, time_offset):
  try:
    ...
    # data point assembly:
    points = [ otime, vehicleid, rpm, mph, throttle, vload, fuelstatus ]
    return   [ {
        'name': 'car',
        'columns': ['time', 'vehicleid', 'rpm', 'mph','throttle','load','fuelstatus'],
        'points': [ points ]}
    ]
  except Exception, e:
